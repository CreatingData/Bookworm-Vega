...
const counttypes = {
    ...
    "TextCount":"# of Texts","TotalTexts":"Total # of Texts","WordsRatio":"Ratio of group A to B","SumWords":"Total in both sets","TextLength":"Mean text length (in words)","MatchesPerText":"Mean hits per matching text","TFIDF":"TFIDF","Dunning":"Dunning Log Likelihood","DunningTexts":"Dunning Log Likelihood (Text count)"}
  ...
  plotAPI(query) {
    ...
    return bookwormFetch(query)
      .then(data => {
          ...
          const plot = new plots[type](query)
              
          const spec = plot.data(data).translate()
